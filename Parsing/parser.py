...
#
# Le fichier de configuration (généré par le script ../initConfig.py) contient le délai (en secondes) entre chaque acquisition
...
if externalLogs:
    sys.stdout = open('../logs/parser.log', 'w')
...
def parser():
    ...
    try:
        ...
        log("critical", "Impossible de lire la configuration, vérifiez le fichier ../config.json. Si besoin, relancez l'utilitaire de création de config ./initConfig.py")
        exit()
    ...
    try: 
        ...
        log("critical", "Impossible de convertir les données depuis le fichier ../config.json, vérifiez vos valeurs.")
        exit()
    ...
    try:
        ...
        log("critical", "Impossible de se connecter à la base de données. Vérifiez l'état du serveur MariaDB et/ou les paramètres de connexion dans ../config.json")
        exit()
    ...
    while True:
        ...
        if i[0] == 0:
            ...
            cursor.execute(addIntensite, (intensite, msTS))
            db.commit()
        ...
        if i[1] == 0:
            ...
            cursor.execute(addBatterie, (tensionCell0, tensionCell1, tensionCell2,  msTS))
            db.commit()
        ...
        if i[2] == 0:
            ...
            with lock:
                speed = actualSpeed
            cursor.execute(addVitesse, (speed, msTS)) #Récupérer la vitesse dans le thread parallèle définit plus haut
        ...
        if i[3] == 0:
            ...
            cursor.execute(addTemperature, (temp4, temp5, temp6, msTS))
            db.commit()
        ...
        if i[4] == 0:
            ...
            cursor.execute(addAccelerateur, (posAcc, msTS))
            db.commit()

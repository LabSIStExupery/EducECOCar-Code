...
def processing():
    ...
    while True:
        ...
        cursor.execute("SELECT Cell1,Cell2,Cell3,TS FROM Battery ORDER BY TS DESC LIMIT 1") #Selectionne la dernière entrée de batterie dans la BDD
        ...
        cursor.execute("INSERT INTO PBattery (Percentage, TS) VALUES (%s, %s)", (percent, TS_B))   #Stocke l'info dans la BDD
        print("[PROCESSING] [DEBUG] [BATTERY] Pourcentage Batterie : {}%".format(percent))
        #Instant Power Processing
        try:
            cursor.execute("SELECT (Battery.Cell1 + Battery.Cell2 + Battery.Cell3)*Current.Current AS Power FROM Battery INNER JOIN Current ON Battery.TS = Current.TS")
            print("[PROCESSING] [DEBUG] [INSTANT POWER] OK")
        except:
            print("******************************UNABLE TO PROCESS INSTANT POWER***********************")
        #Calcul de l'autonomie
        try:
            cursor.execute("SELECT AVG(Current) FROM Current WHERE TS > " + str(timestamp()-(  parameters["AverageOverTime"]*1000))) #Selectionne la moyenne du courant ou tous les TIMESTAMP sont supérieurs au TIMESTAMP actiel moins la durée sur laquelle on se base pour faire la moyenne
            AVGCurrent = cursor.fetchone()[0]   #Consommation moyenne en mAh
            BatteryAuto = parameters["BatteryCapacity"] * percent / 100 #Capacité restante de la batterie en mAh
            BatteryAutoD = Decimal(BatteryAuto)
            Remaining = BatteryAutoD / AVGCurrent * 60 #Autonomie restante en minutes
            cursor.execute("INSERT INTO PAutonomy (Time, TS) VALUES (%s, %s)", (Remaining, timestamp()))
            print("[PROCESSING] Autonomie restante : {}m".format(Remaining))
        except:
            print("[PROCESSING] Unable to process autonomy")
    ...
    #Calcul de la distance parcourue
        ...
        cursor.execute("SELECT TS FROM Speed ORDER BY TS ASC LIMIT 1") # Premier TIMESTAMP enregistré
        FirstTS = cursor.fetchone()[0]

        cursor.execute("SELECT TS FROM Speed ORDER BY TS DESC LIMIT 1") # Dernier TIMESTAMP enregistré
        ...
        cursor.execute("INSERT INTO PDistance (Distance, TS) VALUES (%s, %s)", (Distance, timestamp()))
        ...
        cursor.execute("SELECT Speed FROM Speed ORDER BY TS DESC LIMIT 1") # Derniere vitesse relevée
        Speed = cursor.fetchone()[0]
        cursor.execute("SELECT Current FROM Current ORDER BY TS DESC LIMIT 1") # Derniere Intensité relevée
        Current = cursor.fetchone()[0]
        cursor.execute("SELECT Temp1,Temp2,Temp3,TS FROM Temperature ORDER BY TS DESC LIMIT 1") # Dernieres Températures relevées
        data = cursor.fetchone()
        tempMotor = data[0]
        tempVariator = data[1]
        tempBattery = data[2]
